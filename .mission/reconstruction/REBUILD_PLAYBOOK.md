...
> Last updated: 2026-04-17 | Version: 1.18.0
...
7. **`.mission/architecture/ARCHITECTURE_CURRENT.yaml`** — 20개 모듈(hand-curated registry; src/ 실제 `.ts`는 21개 — `src/index.ts` barrel 포함)의 id, path, responsibility, exports
...
10. **`.mission/interfaces/API_REGISTRY.yaml`** — public API 24개 함수(type export 포함), skill 6개, file_contracts 다수(`.cursorrules`, `AGENTS.md`, `opencode.toml`, `plugin.json`, `SKILL.md`, `.clinerules`, `.continuerules`, `.aider.conf.yml`, `.aider-mission.md`), `package_exports` (라이브러리 subpath) 포함
...
32. `npm test` — 현재 기준 296 tests 전수 통과 (23 test files)
