...
- **Title:** Mission Spec v1.20.1 — docs correction retag (MDR-006 §PATCH)
...
Rev.4 §8 code-actionable 항목(C-1 ~ C-3, F-3, F-4, F-10, C-4, C-2), review follow-up reconciliation(v1.19.3), adopter-compat baseline(v1.20.0), self-description retag(v1.20.1)까지 반영 완료. 남은 축은 user-gated/adoption/future-feature 성격으로 좁혀졌다:
...
- **Rev.5** — 3벤더 재리뷰 (세션 분리 전제, baseline = v1.20.1 HEAD)
