...
> Last updated: 2026-04-17 | Version: 1.16.9

## 현재 상태

- **Title:** Mission Spec v1.16.9 — CURRENT_STATE 3-axis hardening (F-4+C-4)
...
v1.16.x E-series + post-v1.16.6 Codex Rev.2 후속 closure의 저녁~아침 연속 사이클.
...
- [x] `scripts/bump-metadata.js` (D-3, v1.16.3 E-6에서 Title 자동 sync 추가) — `.mission/` Version 헤더 + CURRENT_STATE.md Title 라인 자동 갱신
...
- **C-2** (Codex Rev.2 §2) — `CURRENT_STATE.md` 9/9 PASS를 `evaluateMission()` 실 결과와 mechanically tie (`--verify-live` mode 옵션)
- **C-3** (Codex Rev.2 §4) — `bump-metadata.js` TITLE_RE가 `.mission/` 내 다른 문서의 Title도 rewrite할 수 있는 scope leak. Filename guard 추가 필요
- **F-3** (Claude Rev.1 F-3, Codex Rev.2 §3 재현) — `arch:verify` package_exports 비교를 deep-compare로 확장 (nested conditional exports)
- **F-10** (Gemini Rev.2 Bottom Line) — README.md ko/zh/en을 v1.14.2 ~ v1.16.9 실태로 갱신
- **F-1** (Claude Rev.1 F-1) — cold-build gate의 CI 실전 호출 (C-1 closure 후 viable)
- **E-4** (3벤더 합의 최상위) — 외부 adopter 1+ 확보 — advocacy 영역
- **F-9** — Living Asset Registry 누적 크기 원칙 (MDR-008 후보)
